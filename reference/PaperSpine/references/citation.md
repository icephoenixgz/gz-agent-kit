...
\`paper\_rewriting\_output/citation\_support\_bank.md\` with this table:
...
\- Generate at least \`citation\_target\_count \* 3\` candidates (default: 60).
...
\- Do not use \`\[VERIFY\]\`, \`TODO\`, \`TBD\`, \`pending\`, or empty verification values
 for external-source rows. \`artifact\_check.py\` treats those as FAIL.
...
2\. \*\*Verification pass:\*\* Verify every external-source row and fill \`Verified\`
 plus \`Verification Note\`. Run \`citation\_quality\_audit.py\` and
 \`citation\_verification\_en.py\` where applicable.
...
\`\`\`bash
python scripts/citation\_bank\_check.py paper\_rewriting\_output/citation\_support\_bank.md --target-count 20 --markdown
python scripts/citation\_quality\_audit.py paper\_rewriting\_output --write
python scripts/citation\_verification\_en.py paper\_rewriting\_output/citation\_support\_bank.md --markdown --write
\`\`\`