...
\# rdt-cli is pinned to a git source (PyPI lags upstream) — same pin as the code's \_RDT\_GIT\_SOURCE
...
\- \`--json\` gives the same data machine-readably (\`active\_backend\` per channel)
